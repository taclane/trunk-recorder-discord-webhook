...
int Discord_Bot::system_rates(std::vector<System *> systems, float timeDiff) {
  slash_rates_tr(systems, timeDiff);
...
int Discord_Bot::parse_config(nlohmann::json cfg) {
  ...
  regex_match(this->api_key.c_str(), what, api_regex);
  std::string redacted_api(what[2].first, what[2].second);
  BOOST_LOG_TRIVIAL(info) << log_prefix << "API Key:                " << "******" << redacted_api;
...
void Discord_Bot::start_bot(std::string token) {
  ...
  bot->on_ready([this](const dpp::ready_t &event) {
    ...
    // https://discordlookup.com/permissions-calculator/380175109184
