...
void Discord_Bot::slash_tg(dpp::cluster &bot, const dpp::slashcommand_t &event) {
  ...
  for (std::vector<System *>::iterator it = tr_systems.begin(); it != tr_systems.end(); ++it) {
    ...
    System_impl *sys_impl = (System_impl *)sys;
    ...
    BOOST_LOG_TRIVIAL(info) << log_prefix << " - " << sys->get_talkgroups_file() << sys_impl->channel_file;

    int sys_num = sys->get_sys_num();
    std::string talkgroups_file = sys->get_talkgroups_file();
    std::string channels_file = sys_impl->channel_file;

    if (talkgroups_file != "" || channels_file != "") {
      tgs_found = true;
  ...
  if (!tgs_found) {
    tg_embed
        .set_description("No talkgroups to display.");
  };
